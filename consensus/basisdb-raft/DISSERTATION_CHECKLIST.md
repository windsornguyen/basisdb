...
- [x] Clients assigned unique identifiers
  - Impl: `ClientId` type in `basisdb-raft-io/session.rs`
...
- [x] Leader must have committed entry from current term before serving reads
  - Impl: `Leader::can_serve_reads()` in `basisdb-raft/leader.rs`
...
- [x] Heartbeat round confirms leadership before serving read
  - Impl: `ReadIndexCoordinator` in `basisdb-raft-io/read_index.rs`
...
Run: `cargo test --package basisdb-raft --package basisdb-raft-io`
