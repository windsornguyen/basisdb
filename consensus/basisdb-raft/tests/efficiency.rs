...
impl TestCluster {
    ...
    fn term(&self, id: NodeId) -> u64 {
        self.nodes.get(&id).map_or(0, basisdb_raft::RaftNode::term)
    }

    fn max_term(&self) -> u64 {
        self.nodes.values().map(basisdb_raft::RaftNode::term).max().unwrap_or(0)
    }
...
fn stale_messages_no_persistence() {
    ...
    let stale = Message {
        ...
        to: NodeId(0),
        term: 2, // Old term
        payload: Payload::VoteRequest(basisdb_raft::raft::VoteRequest {
            last_log_index: 0,
            last_log_term: 0,
        }),
    };
