...
use basisdb_raft::raft::{AppendResponse, Config, Message, Payload, RaftNode};
use basisdb_raft::{Command, LogIndex, NodeId};
...
impl TestCluster {
    ...
    fn commit_index(&self, id: NodeId) -> LogIndex {
        self.nodes.get(&id).map_or(0, basisdb_raft::RaftNode::commit_index)
    }
