...
use basisdb_core::{SharedString, fs, install_diagnostics};
use basisdb_node::{NodeConfig, RaftKey, raft_config};
use basisdb_raft::NodeId;
use basisdb_storage::StorageOptions;
...
#[derive(Debug, Parser)]
#[command(name = "bdb", author, version, about = "BasisDB database node", propagate_version = true)]
struct Cli {
...
enum Command {
    /// Boot the BasisDB node using a configuration file.
    Start {
        /// Path to the config file.
        #[arg(long, default_value = "basisdb.toml")]
        config: PathBuf,
    },
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Command::Start { config } => {
            ...
            basisdb_node::launch(node_config)
                .await
...
mod tests {
    ...
    #[test]
    fn invariant_cli_identity_and_default_configuration_agree() {
        use clap::CommandFactory;

        assert_eq!(Cli::command().get_name(), "bdb");
        for command in ["start", "check-config"] {
            let cli = Cli::try_parse_from(["bdb", command]).unwrap();
            let (Command::Start { config } | Command::CheckConfig { config }) = cli.command;
            assert_eq!(config, PathBuf::from("basisdb.toml"));
        }
    }
    ...
    fn example_configuration_is_valid() {
        let config = parse_node_config(include_str!("../../basisdb.example.toml")).unwrap();
    ...
    fn invariant_peer_ids_are_unique() {
        let contents = include_str!("../../basisdb.example.toml").replace(
    ...
    fn invariant_raft_key_is_256_bits() {
        let contents = include_str!("../../basisdb.example.toml")
